...
for dir_name in dir_names:
    ...
    for fullname in sorted(glob(os.path.join(dir_name, '*.csv'))):
        ...
        save_dir_name = '../SkewT_LogP/{0}skew_T-log_P_{2}C_{3}C_{4}_{5}hPa_{1}/'\
            .format(dir_name[3:],obs_year, xxlim[0], xxlim[1], yylim[0], yylim[1])
        ...
        while s_start_date < s_end_date : 
            ...
            s_start_date = s_start_date + relativedelta(hours=12)
            ...
            and os.path.isfile('{0}{1}{2}_{3}_student.csv'.format(dir_name, save_dir_name, filename_el[-5], selected_time[:13])) :
                ...
                try : 
                    ...
                    skew.plot_dry_adiabats(t0=np.arange(-50, 260, 2) * units.degC, 
                                           color='orange', linestyle='-', linewidth=0.3)
                    ...
                    skew.plot_moist_adiabats(t0=np.arange(-50, 160, 2) * units.degC, 
                                             color='green', linestyle='-', linewidth=0.3)
                    skew.plot_moist_adiabats(color='green', linestyle='-', linewidth=1.5, 
                                             label='saturation adiabat')
                    ...
                    skew.plot_mixing_lines(w=mixing_ratio_ticks, color='blue', 
                                           linestyle='--', linewidth=1.0, label='mixing ratio')
                    
                    plt.text(-46.5, 1062, 'mixing ratio', 
                             horizontalalignment='left', verticalalignment='center', 
                             fontsize=9, color='blue')
                    plt.text(-41.2, 1062, '{0}'.format(mixing_ratio_ticks[2][0]*1000), 
                             horizontalalignment='left', verticalalignment='center', 
                             fontsize=9, color='blue')
                    plt.text(-34.2, 1062, '{0}'.format(mixing_ratio_ticks[2][0]*1000), 
                             horizontalalignment='left', verticalalignment='center', 
                             fontsize=9, color='blue')
                    ...
                    plt.text(xxlim[0]+66, yylim[0]+150, '$ \cdotp $ filename : {0}'.format(filename), 
                             horizontalalignment='left', verticalalignment='center', fontsize=12)
                    plt.text(xxlim[0]+67.5, yylim[0]+200, '$ \cdotp $ site : {0}'.format(filename_el[-5]), 
                             horizontalalignment='left', verticalalignment='center', fontsize=10)
                    plt.text(xxlim[0]+69, yylim[0]+260, '$ \cdotp $ time : {0} (UTC)'.format(selected_time[:16]), 
                             horizontalalignment='left', verticalalignment='center', fontsize=10)                    
                    
                    plt.legend(loc='upper left')
                    
                    plt.savefig('{0}{1}{2}_{3}_solution.png'.format(dir_name, save_dir_name, filename_el[-5], 
                        selected_time[:13]), dpi=None, facecolor='w', edgecolor='w',
                        orientation='portrait', papertype=None, format=None,
                        transparent=False, bbox_inches=None, pad_inches=0.1,
                        frameon=None, metadata=None)
                    plt.savefig('{0}{1}{2}_{3}_solution.pdf'.format(dir_name, save_dir_name, filename_el[-5], 
                        selected_time[:13]), dpi=None, facecolor='w', edgecolor='w',
                        orientation='portrait', papertype=None, format=None,
                        transparent=False, bbox_inches=None, pad_inches=0.1,
                        frameon=None, metadata=None)
                    write_log(log_file, '{4} ::: {0}{1}{2}_{3}_solution files are created'\
                              .format(dir_name, save_dir_name, filename_el[-5], selected_time[:13], datetime.now()))
